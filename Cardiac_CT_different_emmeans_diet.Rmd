...
write.xlsx2(posthoc2, paste0(outpath,prefix,"cardiac_anova.xlsx"), sheet =paste0(gsub("_","",gsub("_"," ",colnames(data)[i])),"_HN_emmeans_"),append = TRUE)
...
# pwc <-  pairwise.t.test( data[,i] , data$Geno3, p.adjust.method = "bonferroni")
# 
...
plot_list[[j]]<-ggplot(data, aes(x=Geno3, y=unlist(data[,i]), fill = Sex, alpha=Sex)) +
  ...
  theme_bw() 
  # stat_compare_means(comparisons = my_comparisons, step.increase = 0.2)
...
for (i in c(8:15,19,20,21)) {
  ...
  stat_summary(fun.y=median, geom="point", size=2, color="black", position=dodge) +
  theme(legend.position="bottom")+
        # geom_signif(comparisons = list(c("CTRL", "HFD")), 
...
for (i in c(8:15,19,20,21)) {
  ...
  panel_border() + 
  theme_bw()+
  labs(x = "Diet", y = paste0(gsub("_"," ",colnames(data)[i])), title = paste0(gsub("_"," ",colnames(data)[i])) ) +
  theme(legend.position="bottom")
...
ggsave(paste0(outpath,"TestDietLines_" ,gsub("_"," ",colnames(data)[i]), "042623.pdf"), plot = last_plot(), device='pdf', scale=1, width=mywidth, height=myheight, unit=c("in"), dpi=200)
j=j+1
}
